...
public final class TpaCommands extends ClanSBaseCommand {
    ...
    public @Nullable List<String> primarycommand() {
        return List.of("tpa", "tpahere", "tpaccept", "tpadeny");
    }

    public RootCommandNode<CommandSourceStack> register(CommandDispatcher<CommandSourceStack> dispatcher, String rootLiteral) {
        if (rootLiteral.equals("tpa")) {
            dispatcher.register(tpa(rootLiteral));
        } else if (rootLiteral.equals("tpahere")) {
            dispatcher.register(tpahere(rootLiteral));
        } else if (rootLiteral.equals("tpaccept")) {
            dispatcher.register(tpaccept(rootLiteral));
        }else if (rootLiteral.equals("tpadeny")) {
            dispatcher.register(tpadeny(rootLiteral));
        }
    ...
    private LiteralArgumentBuilder<CommandSourceStack> tpadeny(String command) {
        return Commands.literal(command)
                ...
                .executes(ctx -> acceptLogic(ctx, null, true)); // Ohne ID -> Neueste Anfrage
    }
