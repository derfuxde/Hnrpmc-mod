...
public class ChatBlock {
    ...
    @Deprecated
    public boolean sendBlock(CommandSourceStack player) {
        return sendBlock(player, null, 0);
    }

    @Deprecated
    public boolean sendBlock(CommandSourceStack player, String prefix) {
        return sendBlock(player, prefix, 0);
    }

    public boolean sendBlock(CommandSourceStack player, int amount) {
        return sendBlock(player, null, amount);
    }

    boolean sendBlock(CommandSourceStack player, String prefix, int amount) {
        if (player == null) {
            return false;
        }
        ...
        player.getPlayer().sendSystemMessage(Component.literal(Arrays.toString(messages.toArray(new String[0]))));
        return !rows.isEmpty();
    }
    ...
    private static double msgLength(String str) {
        double length = 0;
        str = ChatFormatting.stripFormatting(str);
     ...
     */
    public static void saySingle(CommandSourceStack receiver, String msg) {
        if (receiver == null) {
            return;
        }

        receiver.getPlayer().sendSystemMessage(Component.literal(colorize(new String[]{cropRightToFit(msg, LINE_LENGTH)})[0]));
    }
     ...
     * @param msg      the message
     */
    public static void sendMessage(@Nullable CommandSourceStack receiver, @NotNull String msg) {
        if (receiver == null) {
            return;
        }

        receiver.getPlayer().sendSystemMessage(Component.literal(ChatUtils.parseColors(msg)));
    }
    ...
    public static void sendMessageKey(@Nullable CommandSourceStack receiver, @NotNull String key, @NotNull Object... args) {
    ...
    public static void sendMessageKey(@NotNull ClanPlayer clanPlayer, @NotNull String key, @NotNull Object... args) {
        sendMessageKey(clanPlayer.toPlayer().createCommandSourceStack(), key, args);
    }
     ...
     * @param receiver the receiver
     */
    public static void sendBlank(CommandSourceStack receiver) {
        if (receiver == null) {
            return;
        }

        receiver.getPlayer().sendSystemMessage(Component.literal(" "));
    }
            ...
            for (String msg : message) {
                ...
                for (int x = 0; x < msg.length(); x++) {
                    ...
                    if (msg.codePointAt(x) == 167) {
                        ...
                        if (x < msg.length()) {
                            lastColor = ChatFormatting.WHITE + "" + msg.charAt(x);
                        }
                    }
                }
