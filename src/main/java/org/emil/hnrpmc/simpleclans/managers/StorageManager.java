...
import org.emil.hnrpmc.simpleclans.storage.MySQLCore;
import org.emil.hnrpmc.simpleclans.storage.SQLiteCore;
import org.emil.hnrpmc.simpleclans.utils.ChatUtils;
import org.emil.hnrpmc.simpleclans.utils.YAMLSerializer;
import org.emil.hnrpmc.simpleclans.uuid.UUIDFetcher;
...
public final class StorageManager {
     ...
     */
    public ChatBlock getChatBlock(ServerPlayer player) {
        return chatBlocks.get(player.getName().getString());
    }
     ...
     *
     */
    public void addChatBlock(CommandSourceStack player, ChatBlock cb) {
        chatBlocks.put(player.getPlayer().getName().getString(), cb);
    }
     ...
     */
    @Deprecated
    public void importFromDatabaseOnePlayer(ServerPlayer player) {
        plugin.getClanManager().deleteClanPlayerFromMemory(player.getUUID());

        ClanPlayer cp = retrieveOneClanPlayer(player.getUUID());
        ...
        if (cp != null) {
            ...
            plugin.getLogger().info("ClanPlayer Reloaded: " + player.getName() + ", UUID: " + player.getUUID());
        }
    }
    ...
    private void purgeClans(List<Clan> clans) {
        ...
        for (Clan clan : purge) {
            plugin.getLogger().info(lang("purging.clan", (ServerPlayer) null, clan.getName()));
    ...
    private void purgeClanPlayers(List<ClanPlayer> cps) {
        ...
        for (ClanPlayer cp : purge) {
            plugin.getLogger().info(lang("purging.player.data", (ServerPlayer) null, cp.getName()));
            deleteClanPlayer(cp);
            cps.remove(cp);
        }
    }
            ...
            try {
                while (res.next()) {
                    try {
                        ...
                        String clan_color = res.getString("clan_color");
                    ...
                    try {
                        ...
                        if (!tag.isEmpty()) {
                            ...
                            if (clan != null && clanDB != null) {
                                ...
                                clanReSync.setName(clanDB.getStringName());
    ...
    public void insertClan(Clan clan) {
        ...
        String query = "INSERT INTO `" + getPrefixedTable("clans") + "` (`banner`, `ranks`, `description`, `fee_enabled`, `fee_value`, `verified`, `tag`," +
                ...
                "`packed_bb`, `cape_url`, `clan_color`, `flags`, `balance`) ";
        String values = "VALUES ( '"
                ...
                + Helper.escapeQuotes(clan.getStringName()) + "',"
                ...
                + Helper.escapeQuotes(clan.getClanColor()) + "','"
    ...
    public void updateClanAsync(final Clan clan) {
        new Thread(() -> {
            updateClan(clan);
        }, "SimpleClans-AsyncUpdate-" + clan.getTag()).start();
    }
    ...
    public void updatePlayerNameAsync(final @NotNull ClanPlayer cp) {
        new Thread(() -> {
                updatePlayerName(cp);
        }, "SimpleClans-AsyncUpdate-" + cp.getName()).start();
    }
    ...
    public void updateClan(Clan clan, boolean updateLastUsed) {
        ...
        try (PreparedStatement st = prepareUpdateClanStatement(core.getGConnection())) {
            setValues(st, clan);
            st.executeUpdate();
        } catch (SQLException ex) {
            plugin.getLogger().error(String.format("Error updating Clan %s", clan.getTag()), ex);
        }
    }
    ...
    private PreparedStatement prepareUpdateClanStatement(Connection connection) throws SQLException {
        String sql = "UPDATE `" + getPrefixedTable("clans") + "` SET ranks = ?, banner = ?, description = ?, fee_enabled = ?, fee_value = ?, " +
                ...
                "packed_allies = ?, packed_rivals = ?, packed_bb = ?, balance = ?, flags = ?, clan_color = ? WHERE tag = ?;";
    ...
    private void setValues(PreparedStatement statement, Clan clan) throws SQLException {
        ...
        statement.setString(9, clan.getStringName());
        ...
        statement.setString(18, clan.getClanColor());
    ...
    public void updateClanPlayer(ClanPlayer cp) {
        ...
        try (PreparedStatement st = prepareUpdateClanPlayerStatement(core.getGConnection())) {
            setValues(st, cp);
            st.executeUpdate();
        } catch (SQLException ex) {
            plugin.getLogger().error(String.format("Error updating ClanPlayer %s", cp.getName()), ex);
        }
    }

    public void updateAllClanandPlayers() {
     ...
     */
    @Deprecated
    public void insertKill(ServerPlayer attacker, String attackerTag, ServerPlayer victim, String victimTag, String type) {
        ...
        String values = "VALUES ( '" + attacker.getUUID() + "','" + attacker.getName() + "','" + attackerTag + "','" + victim.getUUID() + "','" + victim.getName() + "','" + victimTag + "','" + type + "');";
        core.executeUpdate(query + values);
    }
    ...
    public void getKillsPerPlayer(final String playerName, final DataCallback<Map<String, Integer>> callback) {
        new Thread(() -> {
            var result = getKillsPerPlayer(playerName);
            SimpleClans.getInstance().getServer().execute(() -> {
                callback.onResultReady(getKillsPerPlayer(playerName));
            });

        }, "SimpleClans-Async-Kills-per-player-" + playerName).start();
    }
    ...
    private Map<String, UUID> fetchUUIDs(List<ClanPlayer> clanPlayers) {
        ...
        try {
            if (SimpleClans.getInstance().getServer().usesAuthentication()) {
            ...
            plugin.getLogger().error("Error fetching UUIDs in bulk: " + ex.getMessage(), ex);
        }
    ...
    public void saveModified() {
        try (PreparedStatement pst = prepareUpdateClanPlayerStatement(core.getGConnection())) {
            ...
            plugin.getLogger().error("Error saving modified ClanPlayers:", ex);
        }
        try (PreparedStatement pst = prepareUpdateClanStatement(core.getGConnection())) {
