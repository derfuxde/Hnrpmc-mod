...
import static org.emil.hnrpmc.simpleclans.managers.SettingsManager.ConfigField.PAGE_LEADER_COLOR;

public class ClanProfile extends Sendable {

    private final Clan clan;

    public ClanProfile(@NotNull SimpleClans plugin, @NotNull CommandSourceStack sender, @NotNull Clan clan) {
        super(plugin, sender);
        this.clan = clan;
    }
    ...
    @Override
    public void send() {
        String message = lang("clan.profile").replace((CharSequence) "%clan_name%", (CharSequence) clan.getName())
                ...
                .replace("%clan_online_count%", String.valueOf(VanishUtils.getNonVanished(sender.getPlayer(), clan).size()))
