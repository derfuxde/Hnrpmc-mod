...
import static org.emil.hnrpmc.simpleclans.SimpleClans.LOGGER;
...
public class ClanPlayer implements Serializable, Comparable<ClanPlayer> {
    ...
    private transient final Set<String> pastClans = new LinkedHashSet<>();
    ...
    public ClanPlayer(String playerName) {
        this(SimpleClans.getInstance().getServer().getProfileCache().get(playerName).get().getId());
    }

    public boolean hasRankPermission(String permName) {
        return SimpleClans.getInstance().getPermissionsManager().has((ServerPlayer) this.toPlayer(), permName);
    }
    ...
    public ClanPlayer(UUID uuid) {
        uniqueId = uuid;
        Player onlinePlayer = SimpleClans.getInstance().getServer().getPlayerList().getPlayer(uuid);
    ...
    public String getLastSeenDaysString() {
        return getLastSeenDaysString((CommandSourceStack) null);
    }
     ...
     * @return a verbal representation of how many days ago a player was last seen
     */
    public String getLastSeenDaysString(@Nullable CommandSourceStack sender) {
        double days = Dates.differenceInDays(new Timestamp(lastSeen), new Timestamp((new Date()).getTime()));

        if (days < 1) {
            return lang("today", sender.getPlayer());
        } else if (Math.round(days) == 1) {
            return lang("1.color.day", sender.getPlayer());
        } else {
            return SimpleClans.lang("many.color.days", sender, Math.round(days));
        }
    }

    public String getLastSeenDaysString(@Nullable Player viewer) {
        return getLastSeenDaysString(viewer);
    }
    ...
    public String getLastSeenString(@Nullable Player viewer) {
        return getLastSeenString(viewer.createCommandSourceStack());
    }

    public String getLastSeenString(@Nullable CommandSourceStack sender) {
        if (!VanishUtils.isVanished(sender.getPlayer(), this)) {
    ...
    public String getRankDisplayName() {
        Clan currentClan = getClan();
        String rid = getRankId(); // Hol die ID, die der Spieler hat

        if (currentClan != null) {
            Rank r = currentClan.getRank(rid);
            if (r != null) {
                return r.getDisplayName();
            } else {
            }
        } else {
            SimpleClans.LOGGER.info("Spieler {} hat gar keinen Clan-Bezug beim Abrufen des Rangs.", getName());
        }
    ...
    public @Nullable ServerPlayer toPlayer() {
        if (uniqueId != null) {
            return SimpleClans.getInstance().getServer().getPlayerList().getPlayer(uniqueId);
        } else {
            return SimpleClans.getInstance().getServer().getPlayerList().getPlayerByName(displayName);
        }
    }
